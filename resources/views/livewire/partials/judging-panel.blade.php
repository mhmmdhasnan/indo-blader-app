        ...
        <div class="col" style="gap:14px;">
            ...
            @if($koCurrentMatch)
                @php
                    ...
                    $total  = (($judgeExec + $judgeStyle + $judgeCreativity + $judgeDiff + $judgeConsistency) / 5) * 10;
                @endphp
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;" class="prof-grid">
                    ...
                    <div class="panel" style="padding:22px;">
                        ...
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px;">
                            <div style="padding:12px;background:var(--bg-2);border-radius:3px;text-align:center;">
                                @if($riderA)
                                    <x-avatar :initials="$riderA->initials" :size="32" />
                                    <p class="label" style="font-size:13px;margin-top:6px;">{{ $riderA->name }}</p>
                                    <span class="mono dim" style="font-size:10px;">RIDER A</span>
                                @else
                                    <span class="mono dim">TBD</span>
                                @endif
                            </div>
                            <div style="padding:12px;background:var(--bg-2);border-radius:3px;text-align:center;">
                                @if($riderB)
                                    <x-avatar :initials="$riderB->initials" :size="32" />
                                    <p class="label" style="font-size:13px;margin-top:6px;">{{ $riderB->name }}</p>
                        ...
                        @foreach([
                            ...
                            <div style="margin-bottom:16px;">
                                <div class="between" style="margin-bottom:6px;">
                                    <span class="mono" style="font-size:11px;letter-spacing:0.12em;">{{ $lbl }}</span>
                                    <span class="display tnum" style="font-size:18px;color:var(--lime);">{{ number_format($$prop, 1) }}</span>
                                </div>
                                <input type="range" min="0" max="10" step="0.1" wire:model.live="{{ $prop }}"
                                    style="width:100%;accent-color:var(--lime);" />
                            </div>
                    ...
                    <div class="panel col halftone" style="padding:22px;gap:12px;">
                        <span class="kicker">HASIL</span>
                        <div class="center col" style="flex:1;gap:6px;text-align:center;">
                            <span class="display tnum text-glow-lime" style="font-size:clamp(60px,10vw,100px);color:var(--lime);line-height:0.8;">{{ number_format($total, 1) }}</span>
                            <span class="mono dim" style="font-size:12px;">/ 100</span>
                        </div>
                        ...
                        <a href="{{ route('live') }}" class="btn btn-ghost btn-sm">Live Board →</a>
                    </div>
                </div>
